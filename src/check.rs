...
impl ConditionalAttribute {
	...
	pub fn required_feature(&self) -> Option<proc_macro2::Literal> {
		...
		if let Some(TokenTree::Ident(ident_parsed)) = ts.next() {
			if not_ident == ident_parsed {
				// parse #![cfg_attr(not(feature = ...), no_std)]
				if let Some(TokenTree::Group(group_parsed)) = ts.next() {
					...
					if let (Some(TokenTree::Ident(feat_ident_parsed)), Some(TokenTree::Punct(equal_punct_parsed)), Some(TokenTree::Literal(req_literal))) =
						(feat_ident, eq_punct, required_literal)
					{
						if feature_ident == feat_ident_parsed && equal_punct.as_char() == equal_punct_parsed.as_char() {
