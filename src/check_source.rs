...
impl PartialOrd for SourceOffense {
	fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
		Some(match (self, other) {
			(Self::MissingNoStdAttribute, Self::MissingNoStdAttribute) => std::cmp::Ordering::Equal,
			(Self::MissingNoStdAttribute, _) => std::cmp::Ordering::Less,
			(_, Self::MissingNoStdAttribute) => std::cmp::Ordering::Greater,
			(Self::UseStdStatement(_), Self::UseStdStatement(_)) => std::cmp::Ordering::Equal
		})
	}
}

impl Ord for SourceOffense {
...
pub struct UseStdStmt {
	src_path: PathBuf,
	#[cfg_attr(not(feature = "proc_macro_spans"), allow(dead_code))]
...
impl UseStdStmt {
	...
	fn statement_str(&self) -> String {
		...
		let line = file.lines().nth(self.item_tree.span().start().line - 1).unwrap().unwrap();
...
impl fmt::Display for UseStdStmt {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		...
		writeln!(
			f,
			"   → {src}:{line}:{column}",
			src = self.src_path.strip_prefix(std::env::current_dir().unwrap()).unwrap().display(),
			line = self.item_tree.span().start().line,
			column = self.item_tree.span().start().column
		)?;
		writeln!(f, "    |")?;

		writeln!(f, "{line_num:<4}|{line}", line_num = self.item_tree.span().start().line, line = line)?;
		...
		if let Some(replacement_suggestion) = replacement_suggestion {
			writeln!(f, "help: Try replacing `{original}` with `{replacement}`.", original = statement_str, replacement = replacement_suggestion)?;
		}
		Ok(())
	}
}
...
pub fn get_crate_support_from_source(main_src_path: &PathBuf) -> CrateSupport {
	...
	let other_source_files_pattern = format!("{}/**/*.rs", main_src_path.parent().unwrap().to_str().unwrap(),);
...
fn check_source(source_path: &PathBuf, is_main_file: bool) -> CrateSupport {
	...
	for use_statement in &use_statements {
		if let syn::UseTree::Path(ref first_path) = use_statement.tree {
		...
		if !contains_always_no_std {
			...
			if !contains_not_test_no_std {
                ...
                let alternative_test_cases: Vec<(&'static str, syn::Attribute)> = vec![
                    ...
                    ("std", syn::parse_quote!(#![cfg_attr(not(all(feature = "std", test)), no_std)])),
                    ("std", syn::parse_quote!(#![cfg_attr(not(all(test, feature = "std")), no_std)])),
                    // handles `rand_isaac` false negative
                    ("serde", syn::parse_quote!(#![cfg_attr(all(not(feature = "serde"), not(test)), no_std)])),
                    ("serde", syn::parse_quote!(#![cfg_attr(all(not(test), not(feature = "serde")), no_std)])),
                    ("serde", syn::parse_quote!(#![cfg_attr(not(all(feature = "serde", test)), no_std)])),
                    ("serde", syn::parse_quote!(#![cfg_attr(not(all(test, feature = "serde")), no_std)])),
                ];
				for (feature_name, attr) in &alternative_test_cases {
					if syntax.attrs.contains(attr) {
						return CrateSupport::OnlyWithoutFeature(feature_name.to_string());
					}
				}
...
fn find_use_std_statement_replacement(path_parts: &[String]) -> Option<Vec<String>> {
	...
	let glob_pattern = format!("{}/*.{}.html", core_dir.to_str().unwrap(), path_parts.last().unwrap());
	let mut glob_files = glob::glob(&glob_pattern).unwrap();
	match glob_files.next().is_some() {
		true => {
			let replacement_path = vec!["core".to_owned()].into_iter().chain(path_parts.iter().skip(1).cloned()).collect();
	...
	let glob_pattern_index = format!("{}/{}/index.html", core_dir.to_str().unwrap(), path_parts.last().unwrap());
