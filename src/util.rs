...
pub fn metadata_run(additional_args: Option<String>) -> Result<Metadata, ()> {
	...
	let meta = serde_json::from_str(stdout).expect("Fetching metadata failed. Please call cargo-no-std from within a cargo project.");
	Ok(meta)
}

pub fn features_from_args(package_id: String, no_default: bool, features_args: Vec<String>) -> Vec<Feature> {
	let mut features = Vec::new();
	if !no_default {
		let mut feature = Feature::new(package_id.clone(), "default".to_owned());
		feature.causes.push(FeatureCause::Default(package_id.clone()));
...
pub fn main_ws_member_from_args<'a>(metadata: &'a Metadata, package_arg: Option<&str>) -> &'a PackageId {
	if metadata.workspace_members.len() == 1 {
		metadata.workspace_members.get(0).unwrap()
	} else {
		let workspace_members = &metadata.workspace_members[..];
		let workspace_packages: Vec<_> = metadata.packages.iter().filter(|p| workspace_members.contains(&p.id)).collect();
		...
		match package_arg {
			Some(package_name) => {
				...
				if member.is_none() {
					println!("⚠️  Unknown package \"{}\". Please provide one of {:?} via --package flag.", package_name, package_names);
					std::process::exit(1);
				}
				&member.unwrap().id
			}
...
pub fn dependencies_to_packages(package: &Package, metadata: &Metadata, dependencies: &[Dependency]) -> Vec<Package> {
	let resolve_node = metadata.resolve.clone().unwrap().nodes.into_iter().find(|n| n.id == package.id).unwrap();
