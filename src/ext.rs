...
impl Feature {
	...
	pub fn print(&self, metadata: &Metadata, offset: usize) {
		...
		println!(
			"{}{}{}{}{}",
			"❯ Caused by feature flag \"".bright_black(),
			self.name.bright_black().underline(),
			"\" in crate \"".bright_black(),
			package_print_name(&self.package_id).bright_black().bold(),
			"\"".bright_black()
		);
	...
	pub fn print(&self, metadata: &Metadata, offset: usize) {
		...
		match self {
			...
			FeatureCause::CliFlag(flag) => {
				print_offset();
				println!("{}{}{}", "❯ Caused by providing CLI --features flag \"".bright_black(), flag.bright_black(), "\"".bright_black());
			}
			FeatureCause::Default(package_id) => {
				print_offset();
				println!(
					"{}{}{}",
					"❯ Caused by implicitly enabled default feature from \"".bright_black(),
					package_print_name(package_id).bright_black().bold(),
					"\"".bright_black()
				)
			}
			FeatureCause::Explicit(package_id) => {
				...
				println!(
					"{}{}{}",
					"❯ Explicitly enabled feature from \"".bright_black(),
					package_print_name(package_id).bright_black().bold(),
					"\"".bright_black()
				)
			}
...
pub trait PackageExt {
	...
	fn dependency_feature_for_feature(&self, metadata: &Metadata, feature: &Feature) -> Option<Feature>;

	fn dependency_features_for_features(&self, metadata: &Metadata, features: &[Feature]) -> Vec<Feature> {
		features
			.iter()
	...
	fn all_dependency_features(&self, metadata: &Metadata, external_features: &[Feature]) -> Vec<Feature> {
...
impl PackageExt for Package {
					...
					.map(|features| {
						features
							...
							.map(|raw_feature| {
								...
								new_feature.causes.push(FeatureCause::Feature(Box::new(feature.clone())));
		...
		self.dependencies
			...
			.flat_map(|dependency| {
				...
				if dependency.uses_default_features {
					let mut feature = Feature::new(dep_package_id, "default".to_owned());
	...
	fn always_on_dependencies(&self) -> Vec<Dependency> {
		self.dependencies.iter().filter(|dep| !dep.optional).map(|n| n.to_owned()).collect()
	}
	...
	fn is_proc_macro(&self) -> bool {
		self.targets.iter().any(|target| target.kind.contains(&"proc-macro".to_string()))
	}
}
...
impl MetadataExt for Metadata {
	fn find_package(&self, package_id: &str) -> Option<&Package> {
		self.packages.iter().find(|package| package.id.repr == package_id)
	}

	fn dependency_package_id(&self, package: &Package, dependency: &Dependency) -> Option<String> {
		let resolve_node = self.resolve.clone().unwrap().nodes.into_iter().find(|n| n.id == package.id).unwrap();
